...
--4
CREATE INDEX lineitem_idx_lineitem ON lineitem(l_returnflag, l_receiptdate);
...
--8
CREATE INDEX nation_idx_n_regionkey_n_nationkey on nation(n_regionkey);
CREATE INDEX region_idx_r_name_r_regionkey ON region(r_name);
CREATE INDEX supplier_idx_s_nationkey_s_acctbal ON supplier(s_nationkey, s_acctbal);

--9
CREATE INDEX supplier_idx_s_nationkey_s_acctbal ON nation(s_nationkey);
